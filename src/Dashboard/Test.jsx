...
const Test = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axiosSecure.get('http://localhost:5000/fullclasses');
  ...
  const handleUpdateStatus = (id, newStatus) => {
    axiosSecure
      .patch(`http://localhost:5000/api/updatestatus/${id}`, { status: newStatus })
      .then((response) => {
  ...
  const handleUpdateStatustwo = (id, newStatus) => {
    axiosSecure
      .patch(`http://localhost:5000/api/updatestatusdenied/${id}`, { status: newStatus })
      .then((response) => {
        if (response.status === 200) {
          const updatedData = data.map((item) => {
            if (item._id === id) {
